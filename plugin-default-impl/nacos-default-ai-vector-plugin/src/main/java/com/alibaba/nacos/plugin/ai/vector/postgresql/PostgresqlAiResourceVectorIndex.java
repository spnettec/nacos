...
/**
 * PostgreSQL pgvector implementation for AI resource embeddings.
 *
 * @author nacos
 */
public class PostgresqlAiResourceVectorIndex implements AiResourceVectorIndex {
    ...
    public boolean available() {
        ...
        try {
            getJdbcTemplate().queryForObject(
                "SELECT COUNT(1) FROM ai_resource_search_embedding_pg WHERE 1=0",
                Integer.class);
            return true;
    ...
    public boolean isResourceVersionReady(String namespaceId, String resourceType,
        ...
        Integer count = getJdbcTemplate().queryForObject(
            "SELECT COUNT(1) FROM ai_resource_search_embedding_pg WHERE namespace_id=? "
                + "AND resource_type=? AND resource_name=? AND resource_version=? "
                + "AND embedding_model=?",
    ...
    public List<AiResourceVectorHit> search(String namespaceId, String embeddingModel,
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT document_id, chunk_id, resource_type, resource_name, "
    ...
    private boolean isPostgresql() {
        ...
        try {
            ...
            return TYPE.equalsIgnoreCase(dataSourceType)

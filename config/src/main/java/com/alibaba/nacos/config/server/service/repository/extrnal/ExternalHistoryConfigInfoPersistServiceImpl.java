        ...
        try {
            ...
            List<String> columns = new ArrayList<>(
                Arrays.asList("id", "data_id", "group_id", "tenant_id", "app_name", "content",
                    "md5", "src_ip", "src_user", "gmt_modified", "op_type", "publish_type",
                    "gray_name", "ext_info", "encrypted_data_key"));
            List<Object> args = new ArrayList<>(
                Arrays.asList(id, configInfo.getDataId(), configInfo.getGroup(), tenantTmp,
                    appNameTmp, configInfo.getContent(), md5Tmp, srcIp, srcUser, time, ops,
                    publishTypeTmp, grayNameTemp, extInfo, encryptedDataKey));
            if (isOracle()) {
                columns.add("nid");
                args.add(UuidUtils.nextId());
            }
            jt.update(historyConfigInfoMapper.insert(columns), args.toArray());

...
public class ExternalStoragePaginationHelperImpl<E> implements PaginationHelper<E> {
    ...
    private Page<E> doFetchPage(final String sqlCountRows, final Object[] countAgrs, final String sqlFetchRows,
            final Object[] fetchArgs, final int pageNo, final int pageSize, final RowMapper<E> rowMapper) {
        checkPageInfo(pageNo, pageSize);
        // Query the total number of current records
        Integer rowCountInt = null;
        if (null != countAgrs) {
            rowCountInt = jdbcTemplate.queryForObject(sqlCountRows, Integer.class, countAgrs);
        } else {
        ...
        List<E> result = jdbcTemplate.query(sqlFetchRows, rowMapper, fetchArgs);
        for (E item : result) {
            page.getPageItems().add(item);
        }
        return page;

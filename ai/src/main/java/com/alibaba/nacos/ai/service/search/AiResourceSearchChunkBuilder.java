...
/**
 * Builds searchable chunks from an search document.
 *
 * @author nacos
 */
public class AiResourceSearchChunkBuilder {
     ...
     */
    public List<AiResourceSearchChunk> buildChunks(AiResourceSearchDocument entry) {
        List<AiResourceSearchChunk> chunks = new ArrayList<>();
        addChunk(chunks, entry, AiResourceSearchConstants.CHUNK_TYPE_DESCRIPTION,
            firstNotBlank(entry.getDescription(), entry.getDisplayName()), null);
        addListChunks(chunks, entry, AiResourceSearchConstants.CHUNK_TYPE_CAPABILITY,
            parseStringList(entry.getCapabilities()));
        addListChunks(chunks, entry, AiResourceSearchConstants.CHUNK_TYPE_REPRESENTATIVE_QUERY,
            parseStringList(entry.getRepresentativeQueries()));
        addListChunks(chunks, entry, AiResourceSearchConstants.CHUNK_TYPE_TAG,
            parseStringList(entry.getTags()));
        Map<String, Object> metadata = parseMap(entry.getMetadata());
        addMetadataChunk(chunks, entry, metadata, AiResourceSearchConstants.CHUNK_TYPE_METADATA_IO,
            "inputTypes", "outputTypes");
        addMetadataChunk(chunks, entry, metadata,
            AiResourceSearchConstants.CHUNK_TYPE_METADATA_RISK,
            "sideEffects", "riskLevel");
        addMetadataChunk(chunks, entry, metadata, AiResourceSearchConstants.CHUNK_TYPE_NOT_FOR,
            "notFor");
        return chunks;
    }
    ...
    public List<AiResourceSearchChunk> buildEnhancementChunks(AiResourceSearchDocument entry,
        ...
        List<AiResourceSearchChunk> chunks = new ArrayList<>();
        for (AiResourceIndexEnhancementChunk enhancement : enhancements) {
    ...
    private String sourceContentChunkType(AiResourceSearchDocument entry,
        ...
        if (AiResourceConstants.RESOURCE_TYPE_SKILL.equals(entry.getResourceType())
            ...
            return AiResourceSearchConstants.CHUNK_TYPE_SKILL_CONTENT;
        }
        if (AiResourceConstants.RESOURCE_TYPE_PROMPT.equals(entry.getResourceType())
            ...
            return AiResourceSearchConstants.CHUNK_TYPE_PROMPT_CONTENT;
        }
        if (AiResourceConstants.RESOURCE_TYPE_MCP.equals(entry.getResourceType())
            ...
            return AiResourceSearchConstants.CHUNK_TYPE_MCP_CONTENT;
        }
        return null;
    }
    ...
    private String sourceContentType(String chunkType) {
        ...
        if (AiResourceSearchConstants.CHUNK_TYPE_PROMPT_CONTENT.equals(chunkType)) {
            return "prompt_content";
        }
        if (AiResourceSearchConstants.CHUNK_TYPE_MCP_CONTENT.equals(chunkType)) {
            return "mcp_spec";
        }
        return "content";
    }
